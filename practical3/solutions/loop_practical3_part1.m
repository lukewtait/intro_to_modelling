...
for trial = 1:numtrials
    ...
    msg = sprintf('Trial %d of %d',trial,numtrials) ; 
    fprintf(msg) ; 
    
    % Some parameters to change
    coherence = 10 ; % coherence of stimulus, -100 (all dots left) to 100 (all
...
%     toc
    ...
    figure(1) ; 
    colval = 0.8*trial/numtrials ; 
    % plot time domain
    subplot(1,2,1)
    hold on
    plot(t, fr1 , '-','Color',[1,colval,colval])
    plot(t, fr2 , '-','Color',[colval,colval,1])
    xlabel("time [ms]")
    grid on
    ...
    hold on
    plot(s1(t>=0), s2(t>=0), 'color',colval*[1,1,1],'LineWidth',1)
%     text(s1(t==0),s2(t==0),'t=0\rightarrow','horizontalalignment','right','fontsize',8)
    ...
    ind1 = find(fr1 >= params.decisionbound,1) ;
    ind2 = find(fr2 >= params.decisionbound,1) ; 
    ...
    elseif ~isempty(ind1) & isempty(ind2)
        ...
        rt(trial) = t(ind1) ; 
    elseif ~isempty(ind1) & ~isempty(ind2)
        [ind,choice1] = min([ind1,ind2]) ; 
        choice(trial) = choice1 ; 
        rt(trial) = t(ind) ; 
    end
